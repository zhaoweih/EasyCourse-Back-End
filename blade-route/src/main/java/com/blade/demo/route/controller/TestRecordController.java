...
import com.blade.demo.route.model.test.*;
...
//            return RestResponse.fail(500, "找不到该投票id");
    ...
    public RestResponse readRec(@BodyParam String recJson) {
        ...
        try {
            ...
            List<AddRec> addRecs = newRec.getRec_json();

            int voteId = newRec.getVoteId();

            NewTest vote = new NewTest().where("id", voteId).find();
    ...
    public RestResponse updateRec(@BodyParam Record record) {
        ...
        if (new NewTest().where("id", record.getVoteId()).find() == null)
            return RestResponse.fail(500, "找不到该投票id");
